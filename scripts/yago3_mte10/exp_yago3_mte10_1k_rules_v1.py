...
def to_command(c):
    command = "PYTHONPATH=. ./bin/hyper-cli.py" \
              " --train data/yago3_mte10_1k/yago3_mte10-train.tsv.gz" \
              " --valid data/yago3_mte10_1k/yago3_mte10-valid.tsv.gz" \
              " --test data/yago3_mte10_1k/yago3_mte10-test.tsv.gz" \
              " --epochs %s" \
              " --optimizer %s" \
              " --lr %s" \
              " --batches %s" \
              ...
              " --rules ./data/yago3_mte10_1k/rules/yago3_mte10-rules.json.gz" \
              " --rules-threshold %s" \
              " --sample-facts %s" \
              " --rules-lambda %s" \
...
def to_logfile(c, dir):
    outfile = "%s/exp_yago3_mte10_1k_rules_v1.%s.log" % (dir, summary(c))
    return outfile
...
dir = 'logs/exp_yago3_mte10_1k_rules_v1/'
