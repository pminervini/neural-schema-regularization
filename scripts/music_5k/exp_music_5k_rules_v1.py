...
def to_command(c):
    command = "PYTHONPATH=. ./bin/hyper-cli.py" \
              " --train data/music_5k/music-train.txt.gz" \
              " --valid data/music_5k/music-valid.txt.gz" \
              " --test data/music_5k/music-test.txt.gz" \
              " --epochs %s" \
              " --optimizer %s" \
              " --lr %s" \
              " --batches %s" \
...
def to_logfile(c, dir):
    outfile = "%s/exp_music_5k_rules_v1.%s.log" % (dir, summary(c))
    return outfile
...
dir = 'logs/exp_music_5k_rules_v1/'
