...
def to_command(c):
    command = "PYTHONPATH=. ./bin/hyper-cli.py" \
              " --train data/yago2_1k/yago2-train.txt.gz" \
              " --valid data/yago2_1k/yago2-valid.txt.gz" \
              " --test data/yago2_1k/yago2-test.txt.gz" \
              " --epochs %s" \
              " --optimizer %s" \
              " --lr %s" \
              " --batches %s" \
              ...
              " --rules ./data/yago2_1k/rules/yago2_1k-rules.json.gz" \
              " --rules-threshold %s" \
              " --sample-facts %s" \
              " --rules-lambda %s" \
...
def to_logfile(c, dir):
    outfile = "%s/exp_yago2_1k_rules_v1.%s.log" % (dir, summary(c))
    return outfile
...
dir = 'logs/exp_yago2_1k_rules_v1/'
